...
import           Language.Haskell.Exts.Syntax (Boxed (..), Exp (..), Pat (..),
                                               Stmt (..), Type (..))
import           Language.Haskell.Interpreter (Extension (..), GhcError (..),
                                               ImportList (..), Interpreter,
                                               InterpreterError (..),
                                               ModuleImport (..),
                                               ModuleQualification (..),
                                               OptionVal (..), as, interpret,
                                               languageExtensions, lift, liftIO,
                                               runInterpreter, set, setImportsF,
                                               typeChecksWithDetails, typeOf)
import           Synthesis.Ast                (genUncurry)
import           Synthesis.Types
import           Synthesis.Utility            (pp)
import           System.Log.Logger            (alertM, criticalM, debugM,
                                               emergencyM, errorM, infoM,
                                               noticeM, warningM)
