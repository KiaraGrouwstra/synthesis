 ...
 where
    R3NNSpec :: forall m symbols rules t batch_size
      . { variant_sizes :: HashMap String Int
        , conditionSpec :: LSTMSpec (m + batch_size * t * (2 * Dirs * H)) (Div m Dirs) NumLayers Dir 'D.Float Dev
 ...
 where
    R3NN :: forall m symbols rules t batch_size
      . { condition_model :: LSTMWithInit (m + batch_size * t * (2 * Dirs * H)) (Div m Dirs) NumLayers Dir 'ConstantInitialization 'D.Float Dev
        ...
        , symbol_emb :: Parameter Dev 'D.Float '[symbols, m]
        -- for each production rule r∈R, an M−dimensional representation: ω(r)∈R^M.
        , rule_emb   :: Parameter Dev 'D.Float '[rules  , m]
        }
...
forwardPass
    :: forall m symbols t rules batch_size
     . R3NN m symbols rules t batch_size
    -> HashMap String Int
    -> Tnsr '[symbols, m]
    -> Expr
