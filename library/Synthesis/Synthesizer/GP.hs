...
module Synthesis.Synthesizer.GP (
    module Synthesis.Synthesizer.GP
) where
...
-- Matérn 5/2 kernel, used in GPyOpt
-- $$k(r) = \sigma^2 (1 + \sqrt{5} r + \\frac53 r^2) \exp(- \sqrt{5} r)$$
matern52 :: Double -> Double -> Tensor -> Tensor -> Tensor
matern52 sigma _length t t' = (sigma'^2) * midterm * exp eterm
    where
        sigma' = asTensor sigma
        r = t - t'
        sqrt5 :: Float = Prelude.sqrt 5.0
        -- where to use length?
        midterm = onesLike r + mulScalar sqrt5 r + 5/3 * pow (2 :: Int) r
        eterm = mulScalar sqrt5 (-r)
