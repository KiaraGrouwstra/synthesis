...
program = do
    ...
    -- for each function, a list of type instantiations
    -- TODO: fix bug involving this thing ignoring type constraints
    ...
    -- for each function, for each type instantiation, tuples of input (n) + output types
    let fn_io_type_instantiations :: HashMap String [([Tp], Tp)] = fmap fnTypeIO <$> fn_type_instantiations
    say $ "fn_io_type_instantiations: " ++ show (fmap (first (fmap pp) . second pp) <$> fn_io_type_instantiations)

    -- for each function, for each type instantiation, for each param, the input type as string
    let fn_in_str_type_instantiations :: HashMap String [[String]] = fmap (fmap pp . fst) <$> fn_io_type_instantiations
    ...
    -- let in_type_instantiations :: [Tp] = nubBy (equating pp) . concatMap fst . concat . elems $ fn_io_type_instantiations
    -- let str_instantiations :: [String] = pp <$> in_type_instantiations
    let str_instantiations :: [String] = nub . fmap pp . concatMap fst . concat . elems $ fn_io_type_instantiations
    ...
    -- casting keys to string cuz Type isn't hashable 😐
    -- for each parameter combo type instantiation (as string), a list of sample expressions
    instantiation_inputs :: HashMap String [Expr] <- fromList . zip str_instantiations <$> mapM (genInputs numInputs) str_instantiations
    say $ "instantiation_inputs: " ++ show (fmap pp <$> instantiation_inputs)
    ...
    forM_ kept_fns $ \k -> do
        ...
        say $ "\n" ++ pp (expTypeSig (getLet ast) fn_type)
